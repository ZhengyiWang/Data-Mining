...
filter_thresholds=int(sys.argv[1])
...
rdd=input_file.map(lambda data: data.split(",")).map(lambda a:(a[0],a[1])).persist()
res= rdd.map(lambda a: (a[0], [a[1]])).reduceByKey(lambda a,b: a+b).persist()

baskets= res.filter(lambda x:len(x[1])>filter_thresholds).map(lambda a: set(a[1]))
...
def a_priori(b,item,support):
    basket_list=list(b)
    support_threshold=support*(len(basket_list)/b_length)
